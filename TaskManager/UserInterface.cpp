...
std::time_t parseDate(const MyString& dateString) {
	...
	// Since struct tm's months range from 0-11, subtract 1 from month
	//tm.tm_mon -= 1;
...
void run(TaskManager& tm) {
	...
	while (true) {
		...
		if (commandLine == ">register" || commandLine == "> register") {
			try
			{
				MyString username;
				MyString password;
		...
		else if (commandLine == ">login" || commandLine == "> login") {
			try
			{
				MyString username;
				MyString password;
		...
		else if (commandLine == ">logout" || commandLine == "> logout") {
			tm.logout();
		}
		else if (commandLine == ">exit" || commandLine == "> exit") {
			tm.exit();
			return;
		}
		else if (commandLine == ">add-task" || commandLine == "> add-Task") {
			try
			{
				MyString name;
				MyString due_date;
		...
		else if (commandLine == ">get-task" || commandLine == "> get-task") {
			try
			{
				MyString input;
				line >> input;
		...
		else if (commandLine == ">update-task-name" || commandLine == "> update-task-name") {
			try
			{
				size_t ID;
				MyString name;
		...
		else if (commandLine == ">start-task" || commandLine == "> start-task") {
			try
			{
				size_t id;
				line >> id;
		...
		else if (commandLine == ">update-task-description" || commandLine == "> update-task-description") {
			try
			{
				size_t  id;
				MyString description;
		...
		else if (commandLine == ">delete-task" || commandLine == "> delete-task") {
			try
			{
				size_t  id;
				line >> id;
		...
		//for this to function properly pls press space after every input i could change it somehow but ran out of time so went with this thing <3
		else if (commandLine == ">list-tasks" || commandLine == "> list-tasks") {
			try
			{
				MyString input;
				line >> input;
		...
		else if (commandLine == ">finish-task" || commandLine == "> finish-task") {
			try
			{
				size_t  id;
				line >> id;
		...
		//dashboard
		else if (commandLine == ">remove-task-from-dashboard" || commandLine == "> remove-task-from-dashboard") {
			try
			{
				size_t  id;
				line >> id;
		...
		else if (commandLine == ">add-task-to-dashboard" || commandLine == "> add-task-to-dashboard") {
			try
			{
				size_t  id;
				line >> id;
		...
		else if (commandLine == ">list-dashboard" || commandLine == "> list-dashboard") {
			tm.listDashboardTasks();
		}

		//collaboration
		else if (commandLine == ">add-collaboration" || commandLine == "> add-collaboration") {
			try
			{
		...
		else if (commandLine == ">delete-collaboration" || commandLine == "> delete-collaboration") {
			try
			{
				MyString  collaborationName;
				line >> collaborationName;
		...
		else if (commandLine == ">list-collaborations" || commandLine == "> list-collaborations") {
			tm.listCollaborations();
		}

		else if (commandLine == ">add-user" || commandLine == "> add-user") {
			try
			{
				MyString  collaborationName;
				MyString  userName;
		...
		else if (commandLine == ">assign-task" || commandLine == "> assign-task") {
			try
			{
				MyString  collaborationName;
				MyString  username;
